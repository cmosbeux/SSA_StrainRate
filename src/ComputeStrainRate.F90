...
!------------------------------------------------------------------------------
      ...
      WRITE(*,*) FlowSolverName
      IF (FlowSolverName == 'ssavelocity') THEN
        FlowDOFs = dim ! 2D: 2 (u,v)
        CALL Info('ComputeStrainRate', &
          & 'FlowSolverName is >SSAVelocity<, FlowDOFs is the same as problem dimension', &
          & Level=3)
        FlowDOFs = dim ! Setting FlowDOFs based on problem dimension: 3D: 4 (u,v,w,p), 2D: 3 (u,v,p)
      ELSE
        FlowDOFs = dim + 1 ! 3D: 4 (u,v,w,p), 2D: 3 (u,v,p)
      ENDIF
...
!------------------------------------------------------------------------------
          ...
          LocalVelo = 0.0d0
          DO i=1, FlowDOFs
             LocalVelo(i,1:n) = FlowValues(FlowDOFs*(FlowPerm(NodeIndexes(1:n))-1) + i) ! FlowValues for SSA is (u1,v1,...,un,vn)
          END DO
...
!------------------------------------------------------------------------------
      ...
      U_Integ => IntegStuff % u ! coordinate 1 (x)
      V_Integ => IntegStuff % v ! coordinate 2 (y)
      W_Integ => IntegStuff % w ! coordinate 3 (z) (have to be defined ffor ElmentInnfo)

      S_Integ => IntegStuff % s ! weight of the integration point
      N_Integ =  IntegStuff % n ! numnber of integration points
!
